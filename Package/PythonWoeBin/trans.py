...
if __name__ == "__main__":
    ...
    parser.add_option("-v", "--var", dest="var", help="variable list file, required for woe transform", action="store",
                      type="string")
    parser.add_option("-p", "--drop", dest="drop", help="drop variable list file, optional", action="store", type="string")
    ...
    if (not options.var or not os.path.exists(options.var)) and (not options.bin or not os.path.exists(options.bin)):
        print "You must provide either the fine bin result file (-b .bin) or the coarse bin result variable list file (-v _lst.txt)!"
        exit()
    if not options.out:
