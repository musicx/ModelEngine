...
class Variable:
    ...
    def bin(self, mess, pct, sums):
        if self.type == "char":
            ...
            for bad_name in sums:
                ...
                self.badrates[bad_name].missing = self.badrates[bad_name].mbad / (
                    self.badrates[bad_name].missing + 1e-36)
                self.badrates[bad_name].nonmiss = self.badrates[bad_name].nmbad / (
                    self.badrates[bad_name].nonmiss + 1e-36)
        else:
            ...
            if cum_ind > 0:
                ...
                if math.isinf(nonmiss_sqr) or math.isinf(nonmiss_total) or math.isinf(nonmiss_total ** 2):
                    logging.info(
                        "found infinity when z-scaling variable : {}, using 5th and 95th percentile to cap".format(
                            self.name))
                    ...
                    self.distribution.std = math.sqrt(abs((nonmiss_sqr - 2 * nonmiss_total * self.distribution.mean +
                                                           nonmiss_cnt * self.distribution.mean ** 2) / (nonmiss_cnt - 1 + 1e-36)))
        self.distincts = None
    ...
    def toString(self, names=None, dlm=","):
        ...
        for name in names:
            ...
            line += dlm.join((
                "data", "total", "good", "bad", "weighted total", "weighted good", "weighted bad", "high/value")) + "\n"
            data_ind = 0
            ...
            if (not line and lnum < 10000) or lnum == 10000:
                ...
                for ind in input_vars:
                    ...
                    if input_vars[ind].special:
                        logging.debug(
                            "variable {0} is assigned as {1}".format(input_vars[ind].name, input_vars[ind].type))
                    else:
                        parse_error_num = 0
                        known_error_num = 0
                    ...
                    if input_vars[ind].type == "char" and dist_num > 500:
                        ...
                        logging.info(
                            "variable {0} is DELETED due to too many distinct value".format(input_vars[ind].name))
                    elif dist_num > 0.95 * lnum:
                        logging.info("variable {0} is possibly an ID due to too many distinct value".format(
                            input_vars[ind].name))
            if not line:
                break
            if line.strip() == '':
                ...
                for bad_name in bad_values:
                    ...
                    if variables[val_name].type == "char" and bin_num is None:
                        ...
                        if len(variables[val_name].bins[bad_name]) > 500:
                            discard = True
                            logging.info("variable {0} is DELETED due to too many distinct value".format(
                                variables[val_name].name))
                            break
                    if bad_values[bad_name] == 0:
                    ...
                    if variables[val_name].type == "num" and fvalue > variables[val_name].val_datasets[-1][bad_name][
                        -1].value:
...
if __name__ == '__main__':
    ...
    parser.add_option("-t", "--type", dest="type",
                      help="optional variable type file / json string in format var_name[,k[eep]|d[rop]][,n[um]|c[har]] on each line",
                      action="store", type="string")
    parser.add_option("-x", "--excl", dest="exclude",
                      help="optional variable list for simple exclusion of binning, wildchar supported, separate with ','",
                      action="store", type="string")
    parser.add_option("-n", "--num", dest="num", help="number of variables per batch, default is 100 vars at once",
